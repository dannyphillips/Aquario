...
class TankContainer extends Component {
  ...
  // pull the tankId from the route param, then fetch the tank
  componentDidMount() {
    const { history, match, uid } = this.props;

    if (match.params.tankId.length === 6) {
      if (uid) {
        this.checkVote(uid);
      }
  ...
  get tank() {
    ...
    return firebase.tanks.doc(match.params.tankId);
  }

  get results() {
    // get the results sub-collection on the tank document
    return this.tank.collection('results');
  }
